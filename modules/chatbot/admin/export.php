...
function select($table='')
	{
		...
		$op='database_export_insert';
                if ( isset( $_POST['op'] ) )     $op = $_POST['op'];
		// DB feed
		$sform = new XoopsThemeForm( _MD_CHATBOT_DB_EXPORT, "op", xoops_getenv( 'PHP_SELF' ) );
		$sform -> setExtra( 'enctype="multipart/form-data"' );
// Tables
	// Code to create the tables selector
                $tables = array();
                $tables['chatbot_bot']     = _MI_CHATBOT_BOTS;
                $tables['chatbot_topics']  = _MD_CHATBOT_TOPIC;
                $tables['chatbot_content'] = 'Akali Content';
                $tables['chatbot_eliza']   = 'Eliza Content';

        	$tables_array = $tables;
         	$tables_select = new XoopsFormSelect( '', 'table', $table);
        	$tables_select -> addOptionArray( $tables_array );
        	$tables_tray = new XoopsFormElementTray( _MD_CHATBOT_DB_DATAS, '&nbsp;' );
        	$tables_tray -> addElement( $tables_select );
        	$sform -> addElement( $tables_tray );
        	
// Data conversion
	// Code to define how to export datas
	        $export_radio = new XoopsFormRadio(_MD_CHATBOT_TYPE, 'op', $op);
	        $export_radio->addOption("database_export_insert", _MD_CHATBOT_INSERT.'<br />');
	        $export_radio->addOption("database_export_update", _MD_CHATBOT_UPDATE);
	        $sform -> addElement( $export_radio );

		$button_tray = new XoopsFormElementTray( '', '' );
		$butt_create = new XoopsFormButton( '', '', _MD_CHATBOT_EXPORT, 'submit' );
...
function display( $result, $count, $table ) {
         	...
         	$sform = new XoopsThemeForm( _MD_CHATBOT_DB_DATAS, '', '' );
         	$sform->addElement($javascript01);
		$sform->addElement(new XoopsFormTextArea('Table : '.$table.'<br />Total : '.$count.' '._MD_CHATBOT_DB_DATAS,    '', $result, 20 ), FALSE );
		$sform -> display();
}

function chatbot_table_rows( $table ) {
                Global $xoopsDB;
...
switch ( $op )
{
  case "database_export_insert":
        ...
        chatbot_adminmenu(6, _MD_CHATBOT_INSERT);
        chatbot_statmenu(2, '');
        select($table);
        $datas = create_db_results_insert($table,'id');
  ...
  case "database_export_update":
    	include_once( "admin_header.php" );
        chatbot_adminmenu(6, _MD_CHATBOT_UPDATE);
        chatbot_statmenu(2, '');
        select($table);
